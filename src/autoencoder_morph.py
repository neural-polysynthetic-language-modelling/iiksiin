...
class Tensors:
    def __init__(
        ...
        self.input_dimension_size: int = self.bind(next(iter(self.tensor_dict.values())), get_morpheme_role(next(iter(self.morpheme_dict.values())))).view(
            -1
        ).shape[0]
    ...
    def load_from_pickle_file(tensor_file: str, morpheme_file: str) -> "Tensors":
        ...
        with gzip.open(tensor_file) as f:
            ...
            with open(morpheme_file,'rb') as mfile:
        ...
        for batch_of_morphemes in batch_info:  # type: List[str]
            ...
            for numbered_morpheme in enumerate(
                ...
                tensor[n] = self.bind(
                    ...
                    get_morpheme_role(self.morpheme_dict[morpheme]),
                ).view(-1)
...
def program_arguments():
    ...
    arg_parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to morpheme info file for role vector construction."
    )
    return arg_parser

class SuperUnbindingLoss(torch.nn.modules.loss._Loss):
    def __init__(
        self,
        ...
        self.morphs = morphs
        morph_matrix = []
        self.retreiver = TrueTensorRetreiver(autoencoder)
        for morph in self.alphabet.keys():
            morph_matrix.append(get_morpheme_role(morph))
        self.morph_len = len(morph_matrix[0])
        morph_matrix = torch.stack(morph_matrix)
        self.register_buffer("morph_matrix", morph_matrix)

    def forward(self, input, target):
        unbound = torch.einsum(
            "bvr,mr-> bmv",
...
def main():
    ...
    if args.mode == "train" and args.tensor_file:
        ...
        data: Tensors = Tensors.load_from_pickle_file(args.tensor_file, args.morpheme_file)
